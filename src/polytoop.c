...
static Array* array_new(Allocator* allocator)
{
  Array* arr;
...
static void array_delete(Array* arr)
{
  allocator_free(arr->allocator, arr->values, arr->size * sizeof(void*));
  allocator_free(arr->allocator, arr, sizeof(Array));
}



static void array_append(Array* arr, void* value)
{
  arr->values = allocator_realloc(arr->allocator, arr->values, arr->size * sizeof(void*), (arr->size + 1) * sizeof(void*));
...
static void array_remove(Array* arr, void* value)
{
  int i;
  ...
  if (i < arr->size) {
    memmove(&arr->values[i], &arr->values[i + 1], (arr->size - i - 1) * sizeof(void*));
    arr->values = allocator_realloc(arr->allocator, arr->values, arr->size * sizeof(void*), (arr->size - 1) * sizeof(void*));
    --arr->size;
  }
}
...
static polytoop_Vertex* vertex_new(Polytoop* polytoop, double* pos, int index)
{
  ...
  vertex->index = index;
  vertex->position = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));
...
{
  allocator_free(polytoop->allocator, vertex->position, polytoop->dim * sizeof(double));
...
static Ridge* create_ridge(Polytoop* polytoop, polytoop_Vertex** vertices)
{
  ...
  ridge->facets[0] = NULL;
  ridge->facets[1] = NULL;
  ridge->vertices = allocator_alloc(polytoop->allocator, (polytoop->dim - 1) * sizeof(polytoop_Vertex*));
  memcpy(ridge->vertices, vertices, (polytoop->dim - 1) * sizeof(polytoop_Vertex*));
...
static polytoop_Facet* facet_new(Polytoop* polytoop)
{
  ...
  facet->volume = 0.0;
  facet->centroid = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));
  vector_reset(polytoop->dim, facet->centroid);
  facet->normal = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));
  vector_reset(polytoop->dim, facet->normal);
  facet->ridges = array_new(polytoop->allocator);
  facet->vertices = array_new(polytoop->allocator);
...
{
  array_delete(facet->vertices);
  array_delete(facet->ridges);
  allocator_free(polytoop->allocator, facet->normal, polytoop->dim * sizeof(double));
  allocator_free(polytoop->allocator, facet->centroid, polytoop->dim * sizeof(double));
...
static void facet_addoutside(Polytoop* polytoop, polytoop_Facet* facet, Point* point)
{
  ...
  if (facet->prev) {
    ...
    if (polytoop->firstfacet->outsidehead && polytoop->firstfacet->outsidehead->height > facet->outsidehead->height) {
      /* Move to second: */
...
static void ridge_free(Polytoop* polytoop, Ridge* ridge)
{
  if (polytoop->isdelaunay) {
    allocator_free(polytoop->allocator, ridge->normal, (polytoop->dim - 1) * sizeof(double));
    allocator_free(polytoop->allocator, ridge->centroid, (polytoop->dim - 1) * sizeof(double));
  }
  allocator_free(polytoop->allocator, ridge->vertices, (polytoop->dim - 1) * sizeof(polytoop_Vertex*));
...
static void addfacet(Polytoop* polytoop, polytoop_Facet* facet, Ridge** ridges)
{
  ...
  for (i = 0; i < polytoop->dim; ++i) {
    vector_add(polytoop->dim, facet->centroid, ((polytoop_Vertex*)facet->vertices->values[i])->position);
  }
  vector_scale(polytoop->dim, facet->centroid, 1.0 / (double)polytoop->dim);

  /* Vertex span: */
  dirs = alloca((polytoop->dim - 1) * polytoop->dim * sizeof(double));
  for (i = 0; i < polytoop->dim - 1; ++i) {
    /* Vertex position difference: */
    memcpy(&dirs[i * polytoop->dim], ((polytoop_Vertex*)facet->vertices->values[i + 1])->position, polytoop->dim * sizeof(double));
    vector_subtract(polytoop->dim, &dirs[i * polytoop->dim], ((polytoop_Vertex*)facet->vertices->values[0])->position);
  }
  ...
  for (i = 0; i < polytoop->dim - 1; ++i) {
    ...
    pivot = i;
    maxnrmsq = vector_normsq(polytoop->dim, &dirs[i * polytoop->dim]);
    for (j = i + 1; j < polytoop->dim - 1; ++j) {
      double nrmsq;
      nrmsq = vector_normsq(polytoop->dim, &dirs[j * polytoop->dim]);
      if (nrmsq > maxnrmsq) {
        maxnrmsq = nrmsq;
        pivot = j;
      }
    ...
    if (pivot > i) {
      memswp(&dirs[i * polytoop->dim], &dirs[pivot * polytoop->dim], polytoop->dim * sizeof(double));
    }

    /* Normalize: */
    norm = sqrt(maxnrmsq);
    vector_scale(polytoop->dim, &dirs[i * polytoop->dim], 1.0 / norm);

    /* Orthogonalize: */
    for (j = i + 1; j < polytoop->dim - 1; ++j) {
      ip = vector_ip(polytoop->dim, &dirs[j * polytoop->dim], &dirs[i * polytoop->dim]);
      vector_add_scaled(polytoop->dim, &dirs[j * polytoop->dim], &dirs[i * polytoop->dim], -ip);
    }
  ...
  vector_subtract(polytoop->dim, facet->normal, polytoop->center);
  for (i = 0; i < polytoop->dim - 1; ++i) {
    ip = vector_ip(polytoop->dim, facet->normal, &dirs[i * polytoop->dim]);
    vector_add_scaled(polytoop->dim, facet->normal, &dirs[i * polytoop->dim], -ip);
  }
  vector_normalize(polytoop->dim, facet->normal);
  ...
  for (i = 0; i < polytoop->dim; ++i) {
    ...
    array_append(facet->ridges, ridge);
  }
}
...
static void initialsimplex(Polytoop* polytoop, int npoints, Point** points)
{
  ...
  /* Unit box center: */
  vector_set(polytoop->dim, prevpoint, 0.5);
  ...
  for (i = 1; i < npoints; ++i) {
    memcpy(&span[(i - 1) * polytoop->dim], points[i]->pos, polytoop->dim * sizeof(double));
    vector_subtract(polytoop->dim, &span[(i - 1) * polytoop->dim], points[0]->pos);
  }
  ...
  /* Adjust permutation vector to reflect point order (as opposed to 'span' order): */
  p[0] = 0;
  for (i = 1; i < npoints; ++i) {
    ++p[i];
  }
  ...
  vector_reset(polytoop->dim, polytoop->center);
  for (i = 0; i < polytoop->dim + 1; ++i) {
    vector_add(polytoop->dim, polytoop->center, points[p[i]]->pos);
  }
  vector_scale(polytoop->dim, polytoop->center, 1.0 / (double)(polytoop->dim + 1));
  ...
  for (i = 0; i < polytoop->dim + 1; ++i) {
    ...
    for (j = 0; j < polytoop->dim + 1; ++j) {
      if (j != i) {
        /* Add vertex position: */
        array_append(facet->vertices, vertices[j]);
      }
    }
    ...
    for (j = 0; j < polytoop->dim; ++j) {
      ...
      if (!facetridges[j]) {
        ...
        hashmap_insert(polytoop->newridges, facetridges[j]->vertices, facetridges[j]);
      }
    }
  ...
  /* Create outside sets (assign each remaining vertex to a facet which it 'sees'): */
  for (i = polytoop->dim + 1; i < npoints; ++i) {
    ...
    for (facet = polytoop->firstfacet; facet != NULL; facet = facet->next) {
      ...
      vector_subtract(polytoop->dim, vec, facet->centroid);

      /* Orthogonal distance to facet: */
      ip = vector_ip(polytoop->dim, facet->normal, vec);
...
static void addpoint(Polytoop* polytoop, polytoop_Facet* facet, Point* apex)
{
  ...
  /* Allocations: */
  newfacets = array_new(polytoop->allocator);
  horizonridges = array_new(polytoop->allocator);
  vec = alloca(polytoop->dim * sizeof(double));
  ...
  while (visiblelist) {
    ...
    for (iridge = 0; iridge < facet->ridges->size; ++iridge) {
      /* Ridge i: */
      ...
      if (!neighbour) {
        ...
        vector_subtract(polytoop->dim, vec, neighbour->centroid);

        /* Height of apex above neighbour: */
        ip = vector_ip(polytoop->dim, vec, neighbour->normal);
        ...
        if (ip > EPS) {
          ...
          array_append(horizonridges, ridge);
        }
      }
    }
  ...
  /* Form new facets: */
  for (iridge = 0; iridge < horizonridges->size; ++iridge) {
    /* The horizon ridge: */
    ...
    array_append(facet->vertices, vertex);
    ...
    if (polytoop->merge) {
      ...
      vector_subtract(polytoop->dim, vec, neighbour->normal);

      if (vector_normsq(polytoop->dim, vec) < EPS * EPS) {
        double newvol;
        Ridge* ridge2;
        ...
        for (iridge2 = 0; iridge2 < facet->ridges->size; ++iridge2) {
          ridge2 = facet->ridges->values[iridge2];
          if (ridge2 != horizonridge) {
            array_append(neighbour->ridges, ridge2);
        ...
        array_append(neighbour->vertices, vertex);

        /* Add neighbour centroid and volume: */
        newvol = facet->volume + neighbour->volume;
        vector_scale(polytoop->dim, neighbour->centroid, neighbour->volume / newvol);
        vector_add_scaled(polytoop->dim, neighbour->centroid, facet->centroid, facet->volume / newvol);
        neighbour->volume = newvol;

        /* Disassociate the dividing ridge from the neighbour: */
        array_remove(neighbour->ridges, horizonridge);
        ...
        array_remove(horizonridges, horizonridge);
        --iridge;
  ...
  while (point) {
    ...
    /* Find visible facet: */
    for (ifacet = 0; ifacet < newfacets->size; ++ifacet) {
      facet = newfacets->values[ifacet];
      memcpy(vec, point->pos, polytoop->dim * sizeof(double));
      vector_subtract(polytoop->dim, vec, facet->centroid);
      ip = vector_ip(polytoop->dim, vec, facet->normal);
      if (ip > EPS) {
        /* Outside. */
        point->height = ip;
  ...
  array_delete(horizonridges);
  array_delete(newfacets);
}
...
void polytoop_clear(Polytoop* polytoop)
{
  ...
  allocator_free(polytoop->allocator, polytoop->center, polytoop->dim * sizeof(double));
  allocator_free(polytoop->allocator, polytoop->scales, polytoop->dim * sizeof(double));
  allocator_free(polytoop->allocator, polytoop->shift, polytoop->dim * sizeof(double));
#endif
...
void polytoop_fromplanes(Polytoop* polytoop, int n, int d, double* normals, double* dists)
{
  int i;
  int ifacet;
  int ivert;
  ...
  for (i = 0; i < n; ++i) {
    ...
    dist = dists[i] - vector_ip(d, x, &normals[i * d]);
    assert(dist > 0.0);
    vector_scale(d, &points[i * d], 1.0 / dist);
  }
  ...
  for (facet = recpolytoop->firstfacet, ifacet = 0; facet != NULL; facet = facet->next, ++ifacet) {
    /* Get vertices and centroid: */
    vector_reset(d, centroid);
    for (ivert = 0; ivert < d; ++ivert) {
      polytoop_vertex_getposition(polytoop_facet_getvertex(facet, ivert), &verts[ivert * d]);
      vector_add(d, centroid, &verts[ivert * d]);
    }
    vector_scale(d, centroid, 1.0 / (double)d);

    /* Vertices relative to first vertex: */
    for (ivert = 0; ivert < d - 1; ++ivert) {
      memcpy(&dcmp[ivert * d], &verts[(ivert + 1) * d], d * sizeof(double));
      vector_subtract(d, &dcmp[ivert * d], &verts[0 * d]);
    }
    ...
    dist = vector_ip(d, normal, centroid);
    if (dist < 0.0) {
      vector_negate(d, normal);
      dist *= -1.0;
    }

    /* Construct vertex position: */
    memcpy(&points[ifacet * d], normal, d * sizeof(double));
    vector_scale(d, &points[ifacet * d], 1.0 / dist);
    vector_add(d, &points[ifacet * d], x);
  }
...
void polytoop_frompoints(Polytoop* polytoop, int npoints, int dim, double* orgpoints)
{
  int idim;
  int ipoint;
  int* minindices;
  ...
  polytoop->dim = dim;
  polytoop->isdelaunay = 0;
  polytoop->shift = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));
  polytoop->scales = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));
  polytoop->center = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));
  ...
  boundingbox(npoints, polytoop->dim, orgpoints, minindices, maxindices, minima, maxima);

  /* Scales: */
  memcpy(polytoop->scales, maxima, polytoop->dim * sizeof(double));
  vector_subtract(polytoop->dim, polytoop->scales, minima);
  ...
  for (ipoint = 0; ipoint < npoints; ++ipoint) {
    ...
    for (idim = 0; idim < polytoop->dim; ++idim) {
      points[ipoint]->pos[idim] = (orgpoints[ipoint * polytoop->dim + idim] - polytoop->shift[idim]) / polytoop->scales[idim];
    }
  }
...
void polytoop_delaunay(Polytoop* polytoop, int npoints, int dim, double* orgpoints)
{
  int i;
  int j;
  int k;
  ...
  boundingbox(npoints, dim, orgpoints, minindices, maxindices, polytoop->shift, polytoop->scales);
  polytoop->shift[dim] = 0.0;

  /* Scales: */
  vector_subtract(dim, polytoop->scales, polytoop->shift);
  ...
  for (ipoint = 0; ipoint < npoints + 1; ++ipoint) {
    ...
    points[ipoint]->index = ipoint;
    points[ipoint]->height = 0.0;
    points[ipoint]->pos = allocator_alloc(polytoop->allocator, polytoop->dim * sizeof(double));

    if (ipoint < npoints) {
      /* Transformed point: */
      for (i = 0; i < dim; ++i) {
        points[ipoint]->pos[i] = (orgpoints[ipoint * dim + i] - polytoop->shift[i]) / polytoop->scales[i];
      }

      /* Create paraboloid in extra dimension: */
      points[ipoint]->pos[dim] = recd * vector_normsq(dim, points[ipoint]->pos);
    }
  }

  /* Add point above paraboloid to guarantee full dimensionality: */
  vector_reset(dim, points[npoints]->pos);
  for (ipoint = 0; ipoint < npoints; ++ipoint) {
    vector_add(dim, points[npoints]->pos, points[ipoint]->pos);
  }
  vector_scale(dim, points[npoints]->pos, 1.0 / (double)npoints);
  ...
  for (ipoint = npoints; ipoint >= 0; --ipoint) {
    allocator_free(polytoop->allocator, points[ipoint]->pos, polytoop->dim * sizeof(double));
  ...
  ridge = polytoop->firstridge;
  for (i = 0, ridge = polytoop->firstridge; i < polytoop->nridges; ++i, ridge = ridge->next) {
    ridges[i] = ridge;
  }
...
void polytoop_addvertex(Polytoop* polytoop, double* point)
{
  ...
  vector_subtract(polytoop->dim, pos, polytoop->shift);
  for (idim = 0; idim < polytoop->dim; ++idim) {
  ...
  for (facet = polytoop->firstfacet; facet != NULL; facet = facet->next) {
    ...
    vector_subtract(polytoop->dim, vec, facet->centroid);
    dist = vector_ip(polytoop->dim, vec, facet->normal);
    if (dist > EPS) {
      maxfacet = facet;
      break;
    }
...
void polytoop_print(Polytoop* polytoop)
{
  ...
  vector_reset(d, sv);
  for (facet = polytoop->firstfacet, i = 0; facet != NULL; facet = facet->next, ++i) {

    vector_add_scaled(d, sv, facet->normal, facet->volume);

    printf("facet %d\n", i + 1);
    printf("  volume: %g\n", facet->volume);
    printf("  centroid: ");
    vector_print(polytoop->dim, facet->centroid);
    printf("\n");
    printf("  normal: ");
    vector_print(polytoop->dim, facet->normal);
    printf("\n");

    printf("  vertices:\n");
    for (j = 0; j < facet->vertices->size; ++j) {
      vertex = facet->vertices->values[j];
      vector_print(polytoop->dim, vertex->position);
    }
    printf("\n");
  }
...
void polytoop_interpolate(Polytoop* polytoop, double const* xi, int* indices, double* weights)
{
  ...
  while (1) {
    ...
    for (iridge = 0; iridge < polytoop->dim; ++iridge) {
      ...
      if (ridge->normal == NULL) {
        ...
        for (ivertex = 0; ivertex < polytoop->dim - 1; ++ivertex) {
          memcpy(&verts[ivertex * (polytoop->dim - 1)], ridge->vertices[ivertex]->position, (polytoop->dim - 1) * sizeof(double));
        }

        /* Analyse ridge simplex: */
        ridge->centroid = allocator_alloc(polytoop->allocator, (polytoop->dim - 1) * sizeof(double));
        analysesimplex(polytoop->dim - 1, polytoop->dim - 1, verts, &ridge->volume, ridge->centroid, matq);

        /* Last row of LQ decomposition is the normal, save it: */
        ridge->normal = allocator_alloc(polytoop->allocator, (polytoop->dim - 1) * sizeof(double));
        memcpy(ridge->normal, &matq[(polytoop->dim - 2) * (polytoop->dim - 1)], (polytoop->dim - 1) * sizeof(double));
      }
      ...
      vector_subtract(polytoop->dim - 1, vec, currentfacet->centroid);

      /* Normal wants to be outward pointing: */
      if (vector_ip(polytoop->dim - 1, vec, normal) < 0.0) {
        vector_negate(polytoop->dim - 1, normal);
      }

      /* Ridge centroid to interpolation point: */
      memcpy(vec, xiprime, (polytoop->dim - 1) * sizeof(double));
      vector_subtract(polytoop->dim - 1, vec, centroid);

      /* Height of interpolation point above ridge: */
      h = vector_ip(polytoop->dim - 1, vec, normal);
      ...
      indices[iridge] = ((polytoop_Vertex*)currentfacet->vertices->values[polytoop->dim - 1 - iridge])->index;
  ...
  if (totalweight > 0.0) {
    vector_scale(polytoop->dim, weights, 1.0 / totalweight);
  }
}
...
void polytoop_facet_getnormal(polytoop_Facet* facet, double* normal)
{
  ...
  /* Normalize: */
  vector_normalize(facet->polytoop->dim, normal);
}
...
void polytoop_facet_getcentroid(polytoop_Facet* facet, double* centroid)
{
  ...
  vector_add(facet->polytoop->dim, centroid, facet->polytoop->shift);
}
...
double polytoop_facet_getoffset(polytoop_Facet* facet)
{
  ...
  return vector_ip(facet->polytoop->dim, centroid, normal);
}
...
void polytoop_vertex_getposition(polytoop_Vertex* vertex, double* position)
{
  ...
  vector_add(dim, position, vertex->polytoop->shift);
}
